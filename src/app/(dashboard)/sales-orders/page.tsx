        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th className="text-right px-6 py-3 text-gray-500 font-medium">Total Qty</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {orders?.map(o => (
              <tr key={o.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-3 text-gray-500 capitalize">{o.type_of_order ?? '—'}</td>
                <td className="px-6 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[o.status] ?? 'bg-gray-100 text-gray-700'}`}>
                    {o.status}
                  </span>
                </td>
                <td className="px-6 py-3 text-right font-mono">{o.total_so_qty ?? '—'}</td>
              </tr>
            ))}
            {(!orders || orders.length === 0) && (
              <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">No sales orders yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
