  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/deliveries/new" className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            ...
            {deliveries?.map(d => (
              <tr key={d.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[d.status] ?? 'bg-gray-100 text-gray-700'}`}>{d.status}</span>
                </td>
              </tr>
            ))}
            {(!deliveries || deliveries.length === 0) && (
              <tr><td colSpan={5} className="px-6 py-10 text-center text-gray-400">No delivery orders yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
