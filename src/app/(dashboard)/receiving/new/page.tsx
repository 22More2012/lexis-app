...
export default function NewReceivingPage() {
  ...
  const [form, setForm] = useState({
    receive_no: '',
    type_of_receiving: 'purchase_order',
    wira_po_ref: '',
    so_no: '',
    delivery_ref: '',
    product_name: '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error: err } = await supabase.from('receive_orders').insert({
      receive_no: form.receive_no,
      type_of_receiving: form.type_of_receiving,
      wira_po_ref: form.wira_po_ref || null,
      so_no: form.so_no || null,
      delivery_ref: form.delivery_ref || null,
      product_name: form.product_name,
      quantity: parseFloat(form.quantity) || null,
      remaining_qty: parseFloat(form.remaining_qty) || null,
      batch_number: form.batch_number || null,
      wira_po_remark: form.wira_po_remark || null,
      created_by: user.id,
      status: 'pending',
    })
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/dashboard/receiving" className="text-gray-400 hover:text-gray-600">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-900">New Receipt</h1>
          <p className="text-gray-500 text-sm">Record an incoming product receipt</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={loading}
            className="bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white px-6 py-2.5 rounded-lg font-medium text-sm">
