  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/receiving/new" className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            ...
            {records?.map(r => (
              <tr key={r.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[r.status] ?? 'bg-gray-100 text-gray-700'}`}>{r.status}</span>
                </td>
              </tr>
            ))}
            {(!records || records.length === 0) && (
              <tr><td colSpan={6} className="px-6 py-10 text-center text-gray-400">No receiving records yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
