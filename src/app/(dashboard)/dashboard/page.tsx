...
export default async function DashboardPage() {
  ...
  const stats = [
    { label: 'Sales Orders', value: soCount ?? 0, icon: ShoppingCart, color: 'bg-purple-500', href: '/sales-orders' },
    { label: 'IN Receivings', value: receiveCount ?? 0, icon: PackageCheck, color: 'bg-green-500', href: '/receiving' },
    { label: 'OUT Deliveries', value: deliveryCount ?? 0, icon: PackageMinus, color: 'bg-blue-500', href: '/deliveries' },
    { label: 'Pending Approvals', value: pendingApprovals ?? 0, icon: ClipboardList, color: 'bg-amber-500', href: '/approvals' },
  ]
    ...
    <div className="p-8">
      ...
      {(pendingApprovals ?? 0) > 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8 flex items-center gap-3">
          ...
          <p className="text-amber-800 text-sm">
            ...
            <Link href="/approvals" className="underline font-medium">Review now →</Link>
          </p>
        </div>
      )}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <Link href="/products" className="text-blue-600 text-sm font-medium hover:underline">View all →</Link>
        </div>
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {products?.map(p => (
              <tr key={p.product_name} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-3">
                  <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${Number(p.total_inventory) > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            ...
            {(!products || products.length === 0) && (
              <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-400">No products yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
